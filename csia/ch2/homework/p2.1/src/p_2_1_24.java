import edu.princeton.cs.algs4.StdAudio;
import edu.princeton.cs.algs4.StdIn;
import edu.princeton.cs.algs4.StdOut;

public class p_2_1_24 {

    public static double[] superpose(double[] a, double[] b,
                                     double awt, double bwt)
    {
        double[] c = new double[a.length];
        for (int i = 0; i < a.length; i++) {
            c[i] = a[i]*awt + b[i]*bwt;
        }
        return c;
    }

    public static double[] tone(double hz, double t)
    {
        int SAMPLING_RATE = 44100;
        int n = (int) (SAMPLING_RATE * t);
        double[] a = new double[n+1];
        for (int i = 0; i <= n; i++) {

            a[i] = Math.sin(2 * Math.PI * i * hz / SAMPLING_RATE);
        }
        return a;
    }

    public static double[] note( int pitch, double t)
    {
        double hz = 440 * Math.pow(2, pitch / 12.0);
        StdOut.printf("%4.2f\n", hz);

        double[] a = tone(hz, t);
        double[] hi = tone(4*hz, t);
        double[] lo = tone(hz/4, t);
        double[] h = superpose(hi, lo, 0.5, 0.5);
        return superpose(a, h, 0.5, 0.5);
/*
        double[] h = superpose(a, lo, 0.75, 0.25);
        return superpose(hi, h, 0.25, 0.75);*/

    }

    public static void main(String[] args)
    {
        while (!StdIn.isEmpty())
        {
            int pitch = StdIn.readInt();
            double duration = StdIn.readDouble() * 2;
            double[] a = note(pitch, duration);
            StdAudio.play(a);
        }
    }
}

